...
Reply.prototype.addReplyInput = function(editorline) {
  // 댓글 입력 노드를 삽입하는 함수이다.
  ...
  widgetdom.innerHTML = '<div class="reply" style="margin:0;padding:5px; background-color:#f6f7f9;">'+
    '<div class="reply-img" style="padding:5px; display:inline-block;">'+
    '<img src="' + identicon[user_id] + '"></div>'+
    ...
    '<div id="reply-input-' + rellatreply.counter + '" class="reply-input-cell" style="padding:8px;color:#000;" contenteditable="true" onkeydown="return addReply(event,' + rellatreply.counter + ')" onfocus="return replyinputfocus(event)" tabindex="-1">'+
  ...
  if (typeof editorline == "string") {
      ...
      if (widget !== undefined) {
          ...
          // 해당 줄에 이미 생성된 line widge t이 없는 경우
          // make a widget
...
Reply.prototype.addReply = function (event, idcounter) {
  ...
  if (event.keyCode == 13 || event.which == 13) {
      ...
      rellatreply.counter++;
      if (targetinput.containerParent == targetinput.widget.node) addReplyInput(targetinput.widget.line, targetinput.user_id);
      else addReplyInput(targetinput.containerParent.getAttribute('id'), targetinput.user_id);
