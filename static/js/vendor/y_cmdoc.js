...
function extend (Y) {
  Y.requestModules(['Array']).then(function () {
    class YText extends Y.Array.typeDefinition['class'] {
      ...
      // CodeMirror Doc bind
      unbindCodeMirrorDoc (CodeMirrorDocInstance) {
        var i = this.CodeMirrorDocInstances.findIndex(function (binding) {
          return binding.editor === CodeMirrorDocInstance
        })
        if (i >= 0) {
          var binding = this.CodeMirrorDocInstances[i]
          this.unobserve(binding.yCallback)
          binding.editor.off('changes', binding.CodeMirrorDocCallback)
          this.CodeMirrorDocInstances.splice(i, 1)
        }
      }
      unbindCodeMirrorDocAll () {
        for (let i = this.CodeMirrorDocInstances.length - 1; i >= 0; i--) {
          this.unbindCodeMirrorDoc(this.CodeMirrorDocInstances[i].editor)
        }
      }
      bindCodeMirrorDoc (CodeMirrorDocInstance, options) {
        var self = this
        options = options || {}

        // this function makes sure that either the
        // CodeMirrorDoc event is executed, or the yjs observer is executed
        var token = true
        function mutualExcluse (f) {
          if (token) {
            token = false
        ...
        CodeMirrorDocInstance.setValue(this.toString())

        function CodeMirrorDocCallback (cm, delta) {
          mutualExcluse(function () {
            var start = CodeMirrorDocInstance.indexFromPos(delta.from)
            // apply the delete operation first
            if (delta.removed.length > 0) {
            var delLength = 0
            for (var j = 0; j < delta.removed.length; j++) {
        ...
        CodeMirrorDocInstance.on('change', CodeMirrorDocCallback)

        function yCallback (event) {
          mutualExcluse(function () {
            let from = CodeMirrorDocInstance.posFromIndex(event.index)
            if (event.type === 'insert') {
              let to = from
              CodeMirrorDocInstance.replaceRange(event.values.join(''), from, to)
            } else if (event.type === 'delete') {
              let to = CodeMirrorDocInstance.posFromIndex(event.index + event.length)
              CodeMirrorDocInstance.replaceRange('', from, to)
            }
          })
        }
        this.observe(yCallback)
        this.CodeMirrorDocInstances.push({
          editor: CodeMirrorDocInstance,
          yCallback: yCallback,
          CodeMirrorDocCallback: CodeMirrorDocCallback
        })
      }
